...
HittableList RandomWorld() {
	...
	for (int a = -11; a < 11; ++a)
	{
		for (int b = -11; b < 11; ++b)
		{
			const float chooseMat = Unitform(random);
			const Vec3 center(a + 0.9f * Unitform(random), 0.2f, b + 0.9f * Unitform(random));
			...
			if (Length(center-Vec3(4, 0.2f, 0))>0.9)
			{
				if (chooseMat<0.8f)//diffuse
				{
					world.Add(new Sphere(center, 0.2f, new Lambertian(Vec3(
						Unitform(random) * Unitform(random),
						Unitform(random) * Unitform(random),
						Unitform(random) * Unitform(random)
					))));
				}
				else if (chooseMat < 0.95f)//metal
				{
					world.Add(new Sphere(center, 0.2f, new Metal(
						Vec3(0.5f * (1 + Unitform(random)), 0.5f * (1 + Unitform(random)), 0.5f * (1 + Unitform(random))),
						0.5f * Unitform(random)
					)));
				}
				else//Glass 
				{
...
Vec3 Color(Random& random, const Hittable& world, const Ray& ray, const int depth, const int maxDepth) {
	...
	const Vec3 direction = UniVector(ray.Direction);
	const float t = 0.5f * (direction.y() + 1);
	return (1 - t) * Vec3(1.0f) + t * Vec3(0.5, 0.7f, 1.0f);
}
	...
	const int numThreads, const int threadId) 
{
	...
	for (int j = h-threadId-1; j >= 0; j -= numThreads)
	{
		for (int i = 0; i < w; ++i)
		{
			...
			{
				const float u = float(i + Unitform(random)) / float(w);
				const float v = float(j + Unitform(random)) / float(h);
...
void Application() {
	...
	MultiThreadRayTracing(buffer, world, camera, w, h, samples, bounces);
