...
public:
	explicit Dielectric(const float refractionIndex) :RefractionIndex(refractionIndex) {}
	...
	std::optional<MaterialRay>Scatter(const Ray& ray, const HitRecord& record, Random& random) const override{
		...
		const float reflectedProb = refracted ? Schlick(cosine, RefractionIndex) : 1;
		
		return Unitform(random) < reflectedProb
